...
abstract class SwipeHelperRight(
    ...
    private fun drawButtons(
        ...
        buttons.forEach { button ->
            val width = button.intrinsicWidth / buttons.intrinsicWidth() * Math.abs(dX)
            val right = left - width
            button.draw(
                canvas,
                RectF(right, itemView.top.toFloat(), left.toFloat(), itemView.bottom.toFloat())
            )
    ...
    override fun onChildDraw(
        ...
        if (actionState == ItemTouchHelper.ACTION_STATE_SWIPE) {
            if (dX < 0) {
                ...
                maxDX = Math.max(-buttons.intrinsicWidth(), dX)
...
private fun List<SwipeHelperRight.UnderlayButton>.intrinsicWidth(): Float {
